...
type Row = { id?: string; title: string; description: string; icon: string; image_url: string; sort_order: number };
const empty: Row = { title: "", description: "", icon: "ShoppingBag", image_url: "", sort_order: 0 };
...
const AdminProducts = () => {
  ...
  const save = async () => {
    const { error } = draft.id
      ? await supabase.from("products").update(draft).eq("id", draft.id)
      : await supabase.from("products").insert(draft);
      ...
      <AdminCard>
        <table className="w-full text-sm">
          <thead className="bg-muted/30 text-xs uppercase text-muted-foreground">
            <tr><th className="text-left p-3">Sıra</th><th className="text-left p-3">İkon</th><th className="text-left p-3">Başlık</th><th className="p-3 w-24"></th></tr>
          </thead>
          <tbody>
            {rows.map((r) => (
